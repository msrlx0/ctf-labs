...
('Violet GT-R', 'violet-gt-r', 18850000, 2025, 2800, 'Violet Pearl', 'VCGTR2025P7', 'SELL-GTR-91', 'passed', 'domestic clear', 0, 70, 'Limited-run grand tourer with carbon ceramic brakes, violet stitched interior, and concierge-ready inspection coverage.'),
('Nebula S Coupe', 'nebula-s-coupe', 14200000, 2024, 7400, 'Midnight Silver', 'VCNEB2024S9', 'SELL-NEB-44', 'passed', 'import review complete', 0, 55, 'Silent coupe with adaptive air ride, extended battery warranty, and a clean public checkout profile.'),
('Astra V12', 'astra-v12', 31500000, 2023, 5100, 'Obsidian Purple', 'VCAST2023V12', 'SELL-AST-12', 'manual review', 'import bond pending', 1, 95, 'Rare twelve-cylinder collector car queued for seller-assisted delivery while import bond review is completed.'),
('Phantom LX', 'phantom-lx', 22100000, 2025, 900, 'Graphite Violet', 'VCPHX2025LX', 'SELL-PHX-77', 'passed', 'domestic clear', 0, 80, 'Executive luxury sedan with rear lounge package, low mileage, and extended inspection coverage.'),
('Orion Black Edition', 'orion-black-edition', 26400000, 2024, 3200, 'Black Amethyst', 'VCORI2024BE', 'SELL-ORI-32', 'partner hold', 'seller review required', 1, 99, 'High-demand import held for partner-assisted settlement during the checkout migration window.');

INSERT INTO coupons (code, discount_percent, channel_required, requires_seller_approval, active, note) VALUES
('WELCOME10', 10, 'public_checkout', 0, 1, 'Public welcome offer. Valid for buyer checkout but not a seller settlement override.'),
('PURPLE-STAFF', 40, 'partner_checkout', 1, 1, 'Staff-assisted settlement coupon. Requires approved seller review state.'),
...
('Checkout migration behavior', 'Public checkout and partner checkout share styling, but they do not share the same review cache. Support should compare status, channel, and trace wording before closing buyer reports.', 'public', 'open'),
('Staff coupon confusion', 'Staff settlement coupons still fail in public checkout. Seller-assisted review must be reflected in reservation state before coupon policy changes.', 'public', 'triage'),
('Reservation reference formats', 'Buyers see short reservation ids. Seller desks use internal R-####-V references after a migration sync mirrors the hold for operations review.', 'public', 'open'),
('Migration sync complaint', 'Sync errors are expected to differ when channel context, reservation context, or quote pairing is missing. Similar failures should not be merged without comparing the full response.', 'public', 'open'),
('Admin portal QA', 'The parked admin panel still contains an old QA placeholder marker. It is fake, not a challenge flag, and not part of checkout settlement.', 'public', 'closed');
...
('SEL-VIOLET-109', 'R-1047-V', 'Old sample. Partner review should stay forbidden when the request still looks like the public checkout lane.');
...
(3, 'VCAST2023V12', 'violet://inspection/VCAST2023V12', 'manual-review', 'Astra import requires seller review before settlement policy can change.'),
(5, 'VCORI2024BE', 'violet://inspection/VCORI2024BE', 'partner-hold', 'Partner hold active. Channel policy and reservation state must agree.');
...
('migration-note', 'Legacy quote sync records seller review only when checkout context and reservation state agree.', '127.0.0.1');
