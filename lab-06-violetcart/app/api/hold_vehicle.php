...
if (!$quote || !$reservation || !hash_equals((string)$quote['public_token'], $publicToken)) {
    json_response(['error' => 'hold_context_invalid', 'message' => 'Quote and reservation context are required.'], 403, ['X-Violet-Trace' => 'hold-context-invalid']);
}

if ($channel !== 'partner_checkout') {
    json_response([
        'held' => false,
        'message' => 'Public checkout can reserve a car, but partner hold needs seller channel context.'
...
json_response([
    ...
    'state_key' => $stateKey,
    'message' => 'Partner hold warmed for seller-assisted checkout.'
